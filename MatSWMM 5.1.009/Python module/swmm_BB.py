...
INFLOW      = 205		# [NODE]
FLOODING    = 206		# [NODE]
PRECIP      = 207	    # [SUBCATCHMENT]
RUNOFF      = 208		# [SUBCATCHMENT]

# BB
INFIL       = 209	    # [SUBCATCHMENT]
AVAIL_INFIL = 210
EVAP        = 211	    # [SUBCATCHMENT]
GW_ET       = 212
HEAD        = 213	    # [SUBCATCHMENT]
THETA       = 214	    # [SUBCATCHMENT]
MAXINF      = 215       # [SUBCATCHMENT]
POND        = 216       # [SUBAREA]
...
_ERROR_MSG_PATH = AttributeError("Error: Check .rpt or path to .inp")
...
_ERROR_MSG_GROUND_ELEV = ('Ground elevation is below water table for Subcatch {}')
_WARNING_MSG_THETA = ('Warning: Soil moisture greater than porosity.'
                      '\nSetting THETA = porosity at Subcatch {}.')
# ------------------- GLOBAL PRIVATE VARIABLES -----------------

_swmmSO = CDLL(SO_loc) # Loads the DLL

_swmmSO.swmm_get.restype = c_double # Define the return type of the SO fn

_swmmSO.swmm_get_from_input.restype = c_double # Define the return type
...
_pattern = re.compile(".inp", re.IGNORECASE) # Used to create paths for report/output files
_type_constants = (JUNCTION, SUBCATCH, NODE, LINK, STORAGE, ORIFICE, OUTFALL,)
_unit_constants = (US, SI, DIMENSIONLESS, )
_attribute_constants = (DEPTH, VOLUME, FLOW, SETTING, FROUDE, INFLOW, FLOODING,
                        PRECIP, RUNOFF, INFIL, EVAP, GW_ET, HEAD, MAXINF, THETA,
                        POND, AVAIL_INFIL, RUNON, GW_LEAK, ADD_POND, SUBINFLOW,
                        LINK_AREA)
_report_constants = (NO_REPORT, WRITE_REPORT,)
_input_file_constants = (INVERT, DEPTH_SIZE, STORAGE_A, STORAGE_B, STORAGE_C,
                         LENGTH, ROUGHNESS, IN_OFFSET, OUT_OFFSET,
...
def open_file(inp, msg=False):
    ...
    error = _swmmSO.swmm_open(inp, rpt, out)
    if (error != 0):
        raise _ERROR_MSG_PATH
    if msg:
        print "Opening SWMM  -  OK"
...
def start(write_report, msg=False):
    ...
    _start_time = time()
    error = _swmmSO.swmm_start(write_report)
    if (error != 0):
...
def run_step():
    ...
    error = _swmmSO.swmm_step(_ptrTime)
    if (error != 0):
        raise SystemError ("Error {} ocurred at time {}".format(error, _elapsedTime.value))
...
def get_mass_bal_error():
    ...
    error = _swmmSO.swmm_getMassBalErr(ptrRunoff, ptrFlow, ptrQual)
    if (error != 0):
...
def get(object_id, attribute, unit_system):
    ...
    value =  _swmmSO.swmm_get(object_id, attribute, unit_system)
...
def get_from_input(input_file, object_id, attribute):
    ...
    value = _swmmSO.swmm_get_from_input(input_file, object_id, attribute)
    if value == _ERROR_NFOUND:
        raise _ERROR_MSG_NFOUND
    elif value == _ERROR_TYPE:
        raise _ERROR_MSG_TYPE
...
def get_all(input_file, object_type, attribute=-1, save_file=False):
    ...
    error = _swmmSO.swmm_save_all(input_file, object_type, attribute)
...
def setGW(object_id, attribute, units, new_value):
    ...
    Inputs:  object_id	 (str) -> ID of the object, as saved in SWMM.
             ...
             unit_system (int) -> swmm.py constant, related to the units of the attribute that is going
                                   to be retrieved.
    Outputs: _ (double) -> This is the return code of the error message
    Purpose: sets either gw head or soil moisture, which limits infiltration.
    '''

    new_val = c_double(new_value)
    error = _swmmSO.swmm_setGW(object_id, attribute, units, new_val )
    return error
    if error == _ERROR_INCOHERENT:
        raise _ERROR_MSG_INCOHERENT
    elif error == _ERROR_NFOUND:
        raise _ERROR_MSG_NFOUND
    elif error == _ERROR_GROUND_ELEV:
        raise ValueError(_ERROR_MSG_GROUND_ELEV.format(object_id))
    elif error == _WARNING_THETA:
        print (_WARNING_MSG_THETA.format(object_id))

def setPOND(object_id, attribute, units, new_value):
    ...
    Inputs:  object_id	 (str) -> ID of the object, as saved in SWMM.
             ...
             unit_system (int) -> swmm.py constant, related to the units of the attribute that is going
                                   to be retrieved.
    Outputs: _ (double) -> This is the return code of the error message
    Purpose: DON'T NEED THIS
    '''

    new_val = c_double(new_value)
    error = _swmmSO.swmm_addRUN(object_id, attribute, units, new_val )
    if error == _ERROR_INCOHERENT:
        raise _ERROR_MSG_INCOHERENT
    elif error == _ERROR_NFOUND:
        raise _ERROR_MSG_NFOUND
...
def modify_input(input_file, object_id, attribute, value):
    ...
    error =  _swmmSO.swmm_modify_input(input_file, object_id, attribute, new_value)
...
def finish():
    ...
    save_report()  # Step 6
    # BB ; don't save output folders
    #error = _swmmSO.swmm_save_results()
    close()  # Step 7
    return errors
...
def modify_setting(orifice_id, new_setting):
    ...
    tstep  = c_double(0)
    error = _swmmSO.swmm_modify_setting(orifice_id, target, tstep)
    if error == _ERROR_INCOHERENT:
        raise _ERROR_MSG_INCOHERENT
    elif error == _ERROR_NFOUND:
